...
class op_graph:
    ...
    def traverse_and_calculate_per_op(self, ir_params, x, input_name, device, target, fw=True, bw=False, output_file = "out.csv"):
        ...
        """
        global profile_count,key_op_list
...
def op_store(tmp_op,tmp_args,tmp_params):
    global key_op_list
    if tmp_op.type != "call":
        return
    op_name = tmp_op.id.split('-')[0]
    if op_name not in op_list.keys():
        key_op_list[op_name] = {}
        key_op_list[op_name]['args'] = []
        key_op_list[op_name]['params'] = []
    args_shape = []
    for i in range(tmp_args):
        args_shape.append(list(tmp_args[i].shape))
    key_op_list[op_name]['args'].append(args_shape)
    params_shape = []
    for i in range(tmp_params):
        params_shape.append(list(tmp_params[i].shape))
    key_op_list[op_name]['params'].append(params_shape)
...
def profile_forward_relay_operator(ready_op_node_list, ir_params, x, input_name, device, target, dtype="float32"):
    ...
    op_store(ready_op_node,call_intput_args,ir_params)
    ...
    if len(tmp_param)>1:
        for i in range(len(tmp_param)-1):
            if tmp_param[i].name_hint in ir_params.keys() and tmp_param[i+1].name_hint not in ir_params.keys():
                ...
                break
    op_store(ready_op_node, call_intput_args, ir_params)
    # ready_op_node.performance_data["fw_value"] = '1'
